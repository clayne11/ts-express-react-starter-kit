...
// Made modifications to read node_modules recursively up the filesystem
var fs = require('fs')
...
function readDirUpToRoot(
  currentPath = process.cwd(),
  dirName,
  accumulatedNodeModules = []
) {
  var directoryPath = path.join(currentPath, dirName)

  var nodeModulesInCurrentDirectory = readDir(directoryPath)
  const updatedAccumulatedNodeModules = accumulatedNodeModules.concat(
    nodeModulesInCurrentDirectory
  )

  if (currentPath === getFileSystemRoot()) {
    return updatedAccumulatedNodeModules
  } else {
    var parentDirectory = path.normalize(path.join(currentPath, '..'))

    return readDirUpToRoot(
      parentDirectory,
      dirName,
      updatedAccumulatedNodeModules
    )
  }
}
...
module.exports = function nodeExternals(options) {
  ...
  var nodeModules = modulesFromFile
    ? readFromPackageJson()
    : readDirUpToRoot(process.cwd(), modulesDir).filter(isNotBinary)
